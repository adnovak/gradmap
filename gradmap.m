...
function gradmap(GUI_par, ...               % premenna ktora sa vyuziva na prepinanie medzi uzivatelskym rozhranim a davkovym spracovanim. Pri volani nastroja bez pouzitia uzivatelskeho rozhrania treba uviest 'Run'. [string]
    ...
    ...                                 
    header_lines, ...                       % pocet riadkov hlavicky pre preskocenie / number of headerlines in file [double]
    ...
    input_file, ...                         % cesta k vstupnemu suboru/ suborom v pripade davkoveho spracovania / path to input file (fullfile) [string]
    ...
    critical_value,  ...                    % kriticka hodnota pre upozornenie prekrocenia stanovenej presnosti v mikrogaloch na meter / threshold for rejecting solution in microGals per metre [double]
    ...
    number_of_measured_levels, ...          % pocet meranych urovni / number of measured position (levels) [double]
    ...
...
% open GUI
 ...
 %______________________________________________________________________
            ...
            uicontrol(p1,'Units','normalized','position',[0.02 0.78 0.3 0.15],...
            ...
            uicontrol(p1,'Units','normalized', 'Position',[0.32 0.76 0.48 0.14],...
                'backgroundcolor',[R1 G1 B1],'tag','show_local_path',...
                'Style','Text','string','','FontName','Trebuchet MS','FontSize',8);
    
            % Pocet riadkov hlavicky
            uicontrol(p1,'Units','normalized','Position',[0.74 0.61 0.15 0.12],...
            ...
            uicontrol(p1,'Units','normalized', 'Position',[0.01 0.5 0.5 0.2],...
            ...
            uicontrol(p1,'Units','normalized', 'Position',[0.01 0.33 0.59 0.2],...
                         'backgroundcolor',[R1 G1 B1],...
                         'Style','Text','string','vyska merana v jednotkach:','FontName','Trebuchet MS','FontSize',9);
    
            % Vyber jednotiek meranej vysky
            uicontrol(p1,'Units','normalized','position',[0.74 0.35 0.15 0.2],...
                        'Style','Popupmenu','tag','units_option',...
                        'string','cm|m','value',1,'BackgroundColor','white','FontName','Trebuchet MS','FontSize',8.5);

            % Kriticka hodnota text
            uicontrol(p1,'Units','normalized', 'Position',[0.01 0.15 0.52 0.2],...
                         'backgroundcolor',[R1 G1 B1],...
                         'Style','Text','string','kriticka hodnota v µGal:','FontName','Trebuchet MS','FontSize',9);

            % Kriticka hodnota
            uicontrol(p1,'Units','normalized','Position',[0.74 0.25 0.15 0.12],...
                        'Style','Edit','tag','kriticka_hodnota',...
                        'string','5','backgroundcolor',[R2 G2 B2],...
                        'FontName','Trebuchet MS','FontName','Trebuchet MS','FontSize',8.5);

            % Pocet meranych urovni text
            uicontrol(p1,'Units','normalized', 'Position',[0.01 -0.02 0.5 0.2],...
                         'backgroundcolor',[R1 G1 B1],...
                         'Style','Text','string','pocet meranych urovni','FontName','Trebuchet MS','FontSize',9);

            % pocet meranych urovni
            uicontrol(p1,'Units','normalized','position',[0.74 -0.01 0.15 0.2],...
                        'Style','Popupmenu','tag','pocet_meranych_urovni',...
                        'string','2|3','value',1,'BackgroundColor','white','FontName','Trebuchet MS','FontSize',8.5);
...
% ===== PANEL 2 - Vystupne udaje - dodatocne informacie
            ...
            uicontrol(p2,'Units','normalized', 'Position',[0.03 0.135 0.43 0.25],...
                         'backgroundcolor',[R1 G1 B1],...
        ...
        switch GUI_par
            ...
            case 'Run' 
                ...
                if GUI == 1
                    ...
                    critical_value = str2double(get(findobj('tag','kriticka_hodnota'),'string'));
                    measured_levels = get(findobj('tag','pocet_meranych_urovni'),'value');
